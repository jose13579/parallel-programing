...
void *count_hist(void *struct_hist)
{
	...
	for(j=0;j<struct_histogram->nbins;j++) {
		...
		// __sync_fetch_and_add create a full memory barrier, and copy all datas l_hist to struct_histogram->vet
		if(l_hist[j] > 0)
			__sync_fetch_and_add(&struct_histogram->vet[j], l_hist[j]);
